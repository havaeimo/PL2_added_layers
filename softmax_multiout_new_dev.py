...
class ConvSoftmax(Layer):
    ...
    def set_input_space(self, space):
        ...
        self.desired_space = Conv2DSpace(shape=self.input_space.shape,
                                            num_channels=self.n_classes,
                                            axes=('b', 0, 1,'c'))
        self.output_space = self.desired_space
        if not isinstance(space, Space):
            raise TypeError("Expected Space, got " +
                            str(space)+" of type "+str(type(space)))

        self.input_dim = space.get_total_dimension()
        #self.needs_reformat = not isinstance(space, self.desired_space)
    ...
    def fprop(self, state_below):
        ...
        e_Z = T.exp(Z - Z.max(axis=3, keepdims=True))
        rval = e_Z /e_Z.sum(axis=3, keepdims=True)
        return rval
...
class Conv2VectorSpace(Layer):
    ...
    def set_input_space(self, space):
        ...
        self.output_space = VectorSpace(self.input_space.shape[0]*self.input_space.shape[1]*self.n_classes)
    ...
    def fprop(self, state_below):
        ...
        Z_swaped = Z_swaped.reshape(shape=(self.mlp.batch_size,self.input_space.shape[0]*self.input_space.shape[1]*self.n_classes),ndim=2)      
        return Z_swaped
        #return rval
